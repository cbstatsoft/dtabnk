...
`dtabnk` is a memory-efficient CLI tool to **convert World Bank Open Data CSV/Excel files to panel datasets** in STATA (default), SPSS, and/or R formats. It is designed to process large datasets, including in low-RAM environments. Compatible with the default DataBank layout, with support for other common row/column layouts too.
